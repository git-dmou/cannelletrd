 ...
 * 
 */
public class CertifLogo extends AbstractSpecificTreatment {

	private static final Logger logger = Logger.getLogger(CertifLogo.class);

	public final static String PARAM_CONTAINER_LOGO_DESTINATION = "format.act.container.logo.destination";

	public CertifLogo(ResourcesHandler resourcesHandler, ExportedModule module,
	...
	public void run() throws DetailedException {
		try {
			...
			File file = new File(getModule().getAct().getActFile().getParentFile()
			...
			// écrire
			replaceBy = "<filename>"
					+ getValue(PARAM_CONTAINER_LOGO_DESTINATION)
					+ "</filename>";
			...
			if (file.exists()) {
				logger.debug("Fichier traité : '" + file.getAbsolutePath()
						+ "', remplacement de " + find + " par " + replaceBy);
